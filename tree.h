...
class TreeTemplate {
    ...
    protected: class Node {
        ...
        public: unsigned long long hv1;
        public: unsigned long long hv2;
        public: unsigned long long par_hv1;
        public: unsigned long long par_hv2;
        public: unsigned int index;               //:by ruchi
        public: int level;                     //:by ruchi
    };
    ...
    //stores all the tree ids at eact level
    private: typedef boost::unordered_multimap<int, unsigned int> level2id_type;
    public: level2id_type level2id;  
    ...
    //return true if fake_node :by ruchi
    public: inline int return_level(const unsigned int v) {
        return node(v).level;
    }

    public: inline void set_level(unsigned int v, int lv) {
        if(v==NONODE) ERROR_exit("Error!");
        node(v).level = lv;
    }

    // return node index  :by ruchi
    public: inline unsigned int return_index(const unsigned int v) {
        if(v==NONODE) return NONODE;
        return node(v).index; }

    // update node index   :by ruchi
    public: inline void update_index(const unsigned int v, const unsigned int x) {
        if(v!=NONODE)
            node(v).index =  x;   }

    // return node hv1  :by ruchi
    public: inline unsigned int return_hv1(const unsigned int v) {
        if(v==NONODE) return NONODE;
        return node(v).hv1; }

    // return node par_hv1  :by ruchi
    public: inline unsigned int return_par_hv1(const unsigned int v) {
        if(v==NONODE) return NONODE;
        return node(v).par_hv1; }

    // return node par_hv2  :by ruchi
    public: inline unsigned int return_par_hv2(const unsigned int v) {
        if(v==NONODE) return NONODE;
        return node(v).par_hv2; }

    // return node hv2  :by ruchi
    public: inline unsigned int return_hv2(const unsigned int v) {
        if(v==NONODE) return NONODE;
        return node(v).hv2; }

    // set node hv1   :by ruchi
    public: inline void set_hv1(const unsigned int v,unsigned long long val) { node(v).hv1 = val; }

    // set node par_hv1   :by ruchi
    public: inline void set_par_hv1(const unsigned int v,unsigned long long val) { node(v).par_hv1 = val; }

    // set node par_hv2   :by ruchi
    public: inline void set_par_hv2(const unsigned int v,unsigned long long val) { node(v).par_hv2 = val; }

    // set node hv2   :by ruchi
    public: inline void set_hv2(const unsigned int v,unsigned long long val) { node(v).hv2 = val; }
